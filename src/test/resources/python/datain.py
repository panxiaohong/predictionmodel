...
def l2d(lon1,lat1,lon2,lat2):
	#float->rad
	lon1,lat1,lon2,lat2 = map(math.radians,[lon1,lat1,lon2,lat2])#map(function,sequence)是将sequence中每个元素都执行function操作，math.radians方法讲角度转成弧度	
	#haversine公式
	d_lon = lon2 - lon1
	d_lat = lat2 - lat1
...
with open(file_package+'hotpot.txt','r') as rf:
	for line in rf:
		...
		line_new = line.split(',')
		line_w = []
		if (line_new[6] != '0')and(float(line_new[6]) <2000):#vote num第6位是均价
			print 'flag'
			lon_r = float(line_new[7])#经纬度
			lat_r = float(line_new[8])

			pp_r = pp(lon_r,lat_r)
			line_new.insert(7,str(pp_r[0]))   #line_new[9]-----7,8位是平均房价和人口数
			line_new.insert(8,str(pp_r[1]))   #line_new[10]
			
			tt = 9
			poi_r = poi(lon_r,lat_r)#11到之后依次是各种poi的数量
			for i in range(len(poi_r)-1):
				line_new.insert(9+i,str(poi_r[i]))
				tt = tt+1
			
			gps_r = gps(lon_r,lat_r)
			line_new.insert(tt,str(gps_r[0]))#9,10位是上车下车数
			line_new.insert(tt+1,str(gps_r[1]))
			#print line_new，新的数据集
			line_w.append(line_new[4])#1列是点评人数
			line_w.append(line_new[6])#2位是平均价格
			for i in range(7,len(line_new)-2):
				line_w.append(line_new[i])#排除经纬度，依次是平均房价，人口数，上车数，下车数，poi的数量
			print line_w
...
'''
#--------update res--------	
wf = open(r'E:\social_data_new\res_bj_price\p_data.txt','a')
with open(r'E:\social_data_new\restaurant\Beijing\res_bj_avep.txt','r') as rf:
	for line in rf:
		print '**********************'
		line_new = line.split(',')
		if (line_new[6] != '0')and(float(line_new[6] <2000):
			print 'flag'
			lon_r = float(line_new[7])
			lat_r = float(line_new[8])

			pp_r = pp(lon_r,lat_r)
			line_new.insert(7,str(pp_r[0]))   #line_new[9]
			line_new.insert(8,str(pp_r[1]))   #line_new[10]
			
			poi_t = ''
			poi_r = poi(lon_r,lat_r)
			for i in range(len(poi_r)):
				poi_t = poi_t + str(poi_r[i]) + ' '
			line_new.insert(9,poi_t)   #line_new[11]
			
			gps_t = ''
			gps_r = gps(lon_r,lat_r)
			for i in range(len(gps_r)): 
				for j in range(len(gps_r[i])):
					gps_t = gps_t + str(gps_r[i][j]) + ' '
				gps_t = gps_t + ';'
			line_new.insert(10,gps_t)   #line_new[12]
			print line_new
			wf.writelines(','.join(line_new))
			#wf.writelines('\n')
	wf.close()
	rf.close()
print 'finish'
